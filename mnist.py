...
from ncube import adjacency_matrix
...
def load_dataset():
    ...
    X_train = np.pad(X_train, [[0,0], [0,0], [0, 1024-784]], 'constant')
    X_test = np.pad(X_test, [[0,0], [0,0], [0, 1024-784]], 'constant')
...
def build_mlp(input_var=None):
    ...
    l_in = lasagne.layers.InputLayer(shape=(None, 1, 1024),
                                     input_var=input_var)
    ...
    w1 = theano.shared(floatX(w_init((1024, 1024))))
    mask = adjacency_matrix(10).astype(np.float32)
    cube1 = lasagne.layers.DenseLayer(l_hid1, num_units=1024, nonlinearity=lasagne.nonlinearities.linear, W=w1*mask)
    bn1   = lasagne.layers.batch_norm(cube1)
    w2 = theano.shared(floatX(w_init((1024, 1024))))
    cube2 = lasagne.layers.DenseLayer(bn1, num_units=1024, nonlinearity=lasagne.nonlinearities.linear, W=w2*mask)
    bn2   = lasagne.layers.batch_norm(cube2)
